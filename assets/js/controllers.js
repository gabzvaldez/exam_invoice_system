...
.controller('ReportController', function($scope, Main, $window,$filter) {
  ...
  $scope.get_report = function(){
    ...
    if($scope.rep.report == 'daily' && $scope.rep.type == 'meds' && $scope.rep.med_type == 'stock_out'){
      ...
      // }
    }else if($scope.rep.report == 'daily' && $scope.rep.type == 'stocks'){
        var daily_report_data = {
        'report' : $scope.rep.report,
        'datefrom': $filter('date')($scope.rep.datefrom, "yyyy-MM-dd"),
        'dateto' : $filter('date')($scope.rep.dateto, "yyyy-MM-dd")
      ...
      var daily_report_data = {
    ...
    else if($scope.rep.report == 'yearly' && $scope.rep.type == 'sales'){
       Main.yearly_report_financial().then(function(response){
        $scope.yearly_monthly = response.data;
        // console.log(response);
        })      
    }
       ...
       var m_report_data = {
        ...
        console.log(m_report_data);
      // var year = {
      //   'year' : $scope.rep.year
      // }

       Main.monthly_report_financial(m_report_data).then(function(response){
...
.controller('newCtrl', function($scope, Main, $window, $filter){
  ...
  $scope.selectedItemChanged_c = function(id){
      // console.log(ind);
      // console.log(id);
      // $s

      // Main.get_cust().then(function(response){
      //   console.log(response);
      // })
      $scope.dta.address = id.address;
    ...
    if(confirm("Click OK to submit.")){
      var data = {
        'details' : {
          ...
          'customer' : $scope.dta.customer.name,
          // 'type' : $scope.dta.type,
          'orderno' : orderno,
      ...
      Main.issue_ris(data).then(function(response){
        if(response == 1){
                  $window.open(base_url+'/invoice_sys/Admin/print/'+orderno, '_blank');
          $scope.dta = {};
          $scope.items = [];
 ...
 Main.get_spec_meds1(data).then(function(response){
  ...
  $scope.ret_stock = function(t_id2,mid,desc,qty,itemzs){
    // t_id = 
      ...
      swal({
        title: "You have succesfully removed the item!",
